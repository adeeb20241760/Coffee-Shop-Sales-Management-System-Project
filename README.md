...
## 🔍 Database Architecture (`data_investigation.ipynb`)
...
## 🔍 Data Exploration (`data_investigation.ipynb`)

Prior to data cleaning, a rigorous data quality audit was executed across the entire historical transactional log.

### Completeness

100% Data Density (0 null or missing cells detected across all records).

### Negativity Of Values

Checks were done on:

* `transation_id, store_id, product_id ` : Ensure relational database compliance
* `transaction_qty` : It's logically impossible to have negative quantities
* `unit_price`: It's logically impossible to have negative prices.

No negative values were found across all columns.

### Duplicate Values

No duplicate values were found across all columns due to fact that `transaction_id` is unique.
...
Ensure consistent date (YYYY-MM-DD) & Time (HH:MM:SS) format checks.

#### Chronological Check

Ensures date & time values are not beyond the current date.
